...
public class ModuleOnslaught extends ModuleBase {
	...
	public ModuleOnslaught() {
		...
		super(0, ModOnslaught.MOD_ID);

		PACKET_SERVICE = this.enableNetwork();
	}

	public static File LootTableFile;
	...
	public void onPreInitializationEvent(FMLPreInitializationEvent event) {
		...
		TemplateLoader templateLoader = new TemplateLoader(templateStore::setMobTemplateRegistry,
				templateStore::setInvasionTemplateRegistry, modConfigurationPath, filePathCreator,
				new JsonFileLocator(), new MobTemplateLoader(new MobTemplateAdapter()),
		...
		try {
			filePathCreator.initialize(modConfigurationPath.resolve(ModOnslaught.MOD_ID + "/loot"));
		} catch (IOException e) {
			ModOnslaught.LOG.error("Error creating path: " + ModOnslaught.MOD_ID + "/loot");
			ModOnslaught.LOG.error(e.getMessage(), e);
			throw new RuntimeException(e);
		}

		// set data to be available for the 'WorldMixin'
		LootTableFile = modConfigurationPath.resolve(ModOnslaught.MOD_ID + "/loot").toFile();
//		MinecraftForge.EVENT_BUS.register(new LootTableManagerInjectionEventHandler(
//				new CustomLootTableManagerInjector(modConfigurationPath.resolve(MOD_ID + "/loot").toFile())));

		// replaced by 'EntityLivingBaseMixin'
		// MinecraftForge.EVENT_BUS.register(new LootInjectionEventHandler(new
		...
		MinecraftForge.EVENT_BUS.register(new EntityAIInjectionEventHandler(new EntityAIInjectorBase[] {
				new EntityAIPlayerTargetInjector(), new EntityAIChaseLongDistanceInjector(),
				new EntityAIMiningInjector(), new EntityAIAttackMeleeInjector(), new EntityAICounterAttackInjector(),
				new EntityAIExplodeWhenStuckInjector(), new EntityAILungeInjector(), new EntityAIAntiAirInjector(),
				new EntityAIOffscreenTeleportInjector() }));

		// The entity AI classes to strip from mobs during invasion cleanup
		Class<?>[] entityAIToRemoveOnCleanup = { EntityAIPlayerTarget.class, EntityAIChaseLongDistance.class,
				EntityAIChaseLongDistanceGhast.class, EntityAIOffscreenTeleport.class };
		...
		Function<String, InvasionTemplate> idToInvasionTemplateFunction = (id -> templateStore
				.getInvasionTemplateRegistry().get(id));
		Function<String, MobTemplate> idToMobTemplateFunction = (id -> templateStore.getMobTemplateRegistry().get(id));

		MobTemplateEntityFactory mobTemplateEntityFactory = new MobTemplateEntityFactory(new EffectApplicator(),
		...
		MinecraftForge.EVENT_BUS
				.register(new InvasionUpdateEventHandler(new InvasionUpdateEventHandler.IInvasionUpdateComponent[] {

						// State changes -----------------------------------------------

						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(13,
								new StateChangeWaitingToEligible(eligiblePlayers)),
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(15,
								new StateChangeEligibleToPending(eligiblePlayers, invasionSelectorFunction,
										invasionPlayerDataFactory,
										() -> ModuleOnslaughtConfig.INVASION.MAX_CONCURRENT_INVASIONS,
										new InvasionCounter(), new InvasionTimestampFunction(),
										new InvasionMaxDurationFunction(),
										new InvasionWarningMessageTimestampFunction(idToInvasionTemplateFunction,
												() -> ModuleOnslaughtConfig.INVASION.DEFAULT_MESSAGE_WARNING_TICKS))),
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(17,
								new StateChangePendingToActive()),
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(18,
								new StateChangeActiveToWaiting(new InvasionStopExecutor(
										invasionPlayerTimerValueSupplier, new InvasionFinishedPredicate()))),

						// Wave Timers -------------------------------------------------

						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(19,
								new WaveDelayTimer(new ActiveWavePredicate())),

						// Early End Timer----------------------------------------------

						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(20,
								new EarlyEndTimer(invasionPlayerTimerValueSupplier)),

						// Spawns ------------------------------------------------------

						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(20,
								new Spawner(idToInvasionTemplateFunction,
										new SpawnerWave(invasionSpawnDataConverterFunction,
												new SpawnerMob(spawnSampler, idToMobTemplateFunction,
														mobTemplateEntityFactory, entityInvasionDataInjector),
												new SpawnerMobForced(spawnSampler, idToMobTemplateFunction,
														mobTemplateEntityFactory, deferredSpawnDataList,
														() -> ModuleOnslaughtConfig.INVASION.FORCED_SPAWN_DELAY_TICKS),
												new ActiveMobCounter(deferredSpawnDataList)))),
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(21,
								new DeferredSpawner(entityInvasionDataInjector, spawnPredicateFactory,
										invasionSpawnDataConverterFunction, idToMobTemplateFunction,
										mobTemplateEntityFactory, deferredSpawnDataList)),
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(11,
								new DeferredSpawnEffectApplicator(deferredSpawnDataList,
						...
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(20,
								new DeferredSpawnClientParticlePacketSender(deferredSpawnDataList)),
						new InvasionUpdateEventHandler.InvasionTimedUpdateComponent(20 * 5,
								new InvasionMessageSenderWarning(new InvasionMessageSender(idToInvasionTemplateFunction,
										invasionTemplate -> invasionTemplate.messages.warn.message,
										() -> ModuleOnslaughtConfig.INVASION.DEFAULT_MESSAGE_WARNING))) }));

		MinecraftForge.EVENT_BUS
				.register(new InvasionPlayerDataInitializationHandler(invasionPlayerTimerValueSupplier));

		MinecraftForge.EVENT_BUS.register(new InvasionKillCountUpdateEventHandler(new InvasionKillCountUpdater()));

		MinecraftForge.EVENT_BUS.register(new InvasionCleanupEventHandler(
				new EntityInvasionPeriodicWorldCleanup(() -> ModuleOnslaughtConfig.INVASION.OFFLINE_CLEANUP_DELAY_TICKS,
						new EntityInvasionDataRemover(entityAIToRemoveOnCleanup))));

		MinecraftForge.EVENT_BUS.register(new InvasionMessageEventHandler(
				new InvasionMessageSender(idToInvasionTemplateFunction,
		...
		MinecraftForge.EVENT_BUS.register(new InvasionCommandEventHandler(
				new InvasionCommandExecutor(idToInvasionTemplateFunction,
						invasionTemplate -> invasionTemplate.commands.start, invasionCommandSender),
				new InvasionCommandExecutor(idToInvasionTemplateFunction,
						invasionTemplate -> invasionTemplate.commands.end, invasionCommandSender),
				new InvasionCommandExecutorStaged(idToInvasionTemplateFunction, invasionCompletionPercentageCalculator,
						invasionCommandSender)));

		MinecraftForge.EVENT_BUS.register(new InvasionClientUpdateEventHandler(new InvasionClientHUDUpdateSender(
				() -> ModuleOnslaughtConfig.CLIENT.INVASION_HUD_UPDATE_RANGE, invasionCompletionPercentageCalculator)));
		...
		InvasionCommandStarter invasionCommandStarter = new InvasionCommandStarter(invasionPlayerDataFactory,
				eligiblePlayers);

		InvasionStopExecutor invasionStopActiveExecutor = new InvasionStopExecutor(invasionPlayerTimerValueSupplier,
				invasionPlayerData -> invasionPlayerData
						.getInvasionState() == InvasionPlayerData.EnumInvasionState.Active);

		this.commands = new CommandBase[] { new CommandSummon(idToMobTemplateFunction,
				() -> templateStore.getMobTemplateRegistry().getIdList(), mobTemplateEntityFactory),
				new CommandReload(templateLoader),
				new CommandStartInvasion(invasionCommandStarter, idToInvasionTemplateFunction,
				...
				new CommandStopAllInvasion(invasionStopActiveExecutor), new CommandGetInvasion() };
	}
	...
	public void onClientPreInitializationEvent(FMLPreInitializationEvent event) {
		...
		MinecraftForge.EVENT_BUS
				.register(new InvasionHudRenderEventHandler(new InvasionHudRenderer(this.invasionHudRenderInfoList,
	...
	public void onNetworkRegister(IPacketRegistry registry) {
		...
		registry.register(new SCPacketHudUpdateHandler(this.invasionHudRenderInfoList), SCPacketHudUpdate.class,
				Side.CLIENT);
	}
