...
/**
 * Responsible for executing an invasion state transition from pending to
 * active.
 */
public class StateChangePendingToActive implements InvasionUpdateEventHandler.IInvasionUpdateComponent {
		...
		for (EntityPlayerMP player : playerList.getPlayers()) {
			...
			if (invasionData.getTimestamp() <= worldTime) {
				...
				if (ModuleOnslaughtConfig.DEBUG.INVASION_STATE) {
					String message = String.format("Set invasion state to %s for player %s", "Active",
							player.getName());
					ModOnslaught.LOG.debug(message);
				}

				MinecraftForge.EVENT_BUS.post(new InvasionStateChangedEvent(player,
						InvasionPlayerData.EnumInvasionState.Pending, InvasionPlayerData.EnumInvasionState.Active));
			}
		}
	}
}
