...
public class EntityInvasionPeriodicWorldCleanup {
	...
	public EntityInvasionPeriodicWorldCleanup(LongSupplier offlineCleanupDelayTicksSupplier,
	...
	public void cleanup(long worldTime, List<EntityLiving> entityLivingList,
		...
		for (EntityLiving entity : entityLivingList) {
			...
			if (player == null) {
				...
				if (modTag.hasKey(Tag.INVASION_OFFLINE_TIMESTAMP)) {
					...
					if (worldTime
							- modTag.getLong(Tag.INVASION_OFFLINE_TIMESTAMP) >= this.offlineCleanupDelayTicksSupplier
									.getAsLong()) {
			...
			if (playerData.getInvasionState() != InvasionPlayerData.EnumInvasionState.Active || invasionData == null) {
