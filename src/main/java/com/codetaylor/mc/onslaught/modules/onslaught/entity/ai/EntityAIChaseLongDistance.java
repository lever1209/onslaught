...
public class EntityAIChaseLongDistance extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		double followRange = this.taskOwner.getEntityAttribute(SharedMonsterAttributes.FOLLOW_RANGE)
				.getAttributeValue();
		...
		Vec3d pathVec = this.findTargetBlock(this.taskOwner, navigator, random, originVector, targetVector, distance,
				searchRangeXZ, searchRangeY);

		// if (pathVec == null) {
		// pathVec =
		// RandomPositionGenerator.findRandomTargetBlockTowards(this.taskOwner, (int)
	 ...
	 *
	 * @see RandomPositionGenerator#generateRandomPos(EntityCreature, int, int,
	 *      Vec3d, boolean)
	 */
	@SuppressWarnings("JavadocReference")
	@Nullable
	private Vec3d findTargetBlock(EntityLiving entity, PathNavigate navigator, Random random, Vec3d origin,
			Vec3d target, int distance, int searchRangeXZ, int searchRangeY) {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (navigator.canEntityStandOnPos(blockPos)) {
				...
				float blockPathWeight = (entity instanceof EntityCreature)
