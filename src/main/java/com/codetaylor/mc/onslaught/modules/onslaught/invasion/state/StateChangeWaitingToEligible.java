...
/**
 * Responsible for transitioning a player's invasion state from waiting to
 * eligible.
 */
public class StateChangeWaitingToEligible implements InvasionUpdateEventHandler.IInvasionUpdateComponent {
			...
			if (ticksUntilNextInvasion > 0) {
				...
				if (previousInvasionState != InvasionPlayerData.EnumInvasionState.Eligible) {
					...
					if (ModuleOnslaughtConfig.DEBUG.INVASION_STATE) {
						String message = String.format("Set invasion state to %s for player %s", "Eligible",
								player.getName());
						ModOnslaught.LOG.debug(message);
					}

					MinecraftForge.EVENT_BUS
							.post(new InvasionStateChangedEvent(player, InvasionPlayerData.EnumInvasionState.Waiting,
