...
public class EntityAIMining extends EntityAIBase {
	...
	public void updateTask() {
		...
		this.taskOwner.getLookHelper().setLookPosition(this.attackTarget.posX,
				this.attackTarget.posY + this.attackTarget.getEyeHeight(), this.attackTarget.posZ,
		...
		float blockStrength = this.getBlockStrength(blockState, this.taskOwner, world, this.blockTarget)
				* (this.blockBreakTickCounter + 1);
		...
		if (blockStrength >= 1) {
			...
			navigator.setPath(navigator.getPathToEntityLiving(this.attackTarget),
					this.taskOwner.getMoveHelper().getSpeed());
			this.resetTask();

		} else if (this.blockBreakTickCounter % 5 == 0) {
			world.playSound(
					null, this.blockTarget, blockState.getBlock()
							.getSoundType(blockState, world, this.blockTarget, this.taskOwner).getHitSound(),
	...
	private BlockPos getBlockTarget() {
		...
		for (AxisAlignedBB collisionBox : collisionBoxes) {
			BlockPos pos = new BlockPos(collisionBox.minX + (collisionBox.maxX - collisionBox.minX) * 0.5D,
			...
			if (!world.isAirBlock(pos)) {
				...
				if (!ModuleOnslaughtConfig.CUSTOM_AI.MINING.MINE_WITHOUT_REQUIRED_TOOL
						&& !this.canHarvest(world.getBlockState(pos), this.taskOwner)) {
					continue;
				}
	...
	/**
	 * This is derived from the linked method and altered to support non-player
	 * entities.
	 *
	 * @see net.minecraftforge.common.ForgeHooks#blockStrength(IBlockState,
	 ...
	 * entities.
	 *
	 * @see net.minecraftforge.common.ForgeHooks#canHarvestBlock(Block,
	 *      EntityPlayer, IBlockAccess, BlockPos)
	 */
	 ...
	 * entities.
	 *
	 * @see net.minecraft.entity.player.EntityPlayer#getDigSpeed(IBlockState,
	 *      BlockPos)
	 */
