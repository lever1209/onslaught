...
/**
 * Responsible for injecting the AI melee attack task into entities with the
 * tag.
 */
public class EntityAIAttackMeleeInjector extends EntityAIInjectorBase {
	...
	public void inject(EntityLiving entity, NBTTagCompound tag) {
		...
		if (!(entity instanceof EntityCreature)) {
			ModOnslaught.LOG.error("EntityAIAttackMelee can only be applied to subclasses of EntityCreature, was "
					+ entity.getClass());
			return;
		}
		...
		double speed = this.getDouble(aiTag, Tag.AI_PARAM_SPEED,
				ModuleOnslaughtConfig.CUSTOM_AI.ATTACK_MELEE.DEFAULT_SPEED);
		float attackDamage = (float) this.getDouble(aiTag, Tag.AI_PARAM_ATTACK_DAMAGE,
