...
/**
 * Responsible for injecting the AI explode when stuck task into entities with
 * the tag.
 */
public class EntityAIExplodeWhenStuckInjector extends EntityAIInjectorBase {
	...
	public void inject(EntityLiving entity, NBTTagCompound tag) {
		...
		int explosionDelayTicks = this.getInteger(aiTag, Tag.AI_PARAM_EXPLOSION_DELAY_TICKS,
				config.DEFAULT_EXPLOSION_DELAY_TICKS);
		float explosionStrength = (float) this.getDouble(aiTag, Tag.AI_PARAM_EXPLOSION_STRENGTH,
				config.DEFAULT_EXPLOSION_STRENGTH);
		boolean explosionCausesFire = this.getBoolean(aiTag, Tag.AI_PARAM_EXPLOSION_CAUSES_FIRE,
				config.DEFAULT_EXPLOSION_CAUSES_FIRE);
		boolean explosionDamaging = this.getBoolean(aiTag, Tag.AI_PARAM_EXPLOSION_DAMAGING,
				config.DEFAULT_EXPLOSION_DAMAGING);

		entity.tasks.addTask(priority, new EntityAIExplodeWhenStuck(entity, sightRequired, rangeRequired, rangeMin,
				rangeMax, explosionDelayTicks, explosionStrength, explosionCausesFire, explosionDamaging));
	}
}
