...
public class InvasionHudRenderer {
	...
	public InvasionHudRenderer(List<InvasionHudRenderInfo> invasionHudRenderInfoList, IntSupplier xPositionSupplier,
			IntSupplier yPositionSupplier, IntSupplier widthSupplier, Supplier<int[]> barColorSupplier) {
	...
	public void render() {
		...
		if (this.debugRender) {
			...
			String[] invasionNames = new String[] { "My Invasion", "Some other invasion", "Invasion with crazy things",
					"ALL CAPS INVASION" };
			for (int i = 0; i < 4; i++) {
	...
	private void render(int index, InvasionHudRenderInfo info) {
		...
		// Gui.drawRect(offsetX, offsetY, offsetX + cardWidth, offsetY + cardHeight, new
		// Color(1, 0, 0,
		// 0.25f).getRGB
		// ());
...

		{
			if (info.invasionName.length() > 0) {
				...
				String trimmed = minecraft.fontRenderer.trimStringToWidth(full, cardWidth - headSize - cardMargin - 2);
