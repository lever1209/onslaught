...
public class EntityAIAttackMelee extends net.minecraft.entity.ai.EntityAIAttackMelee {
	...
	private void attackEntityAsMob(EntityCreature entityAttacker, @Nonnull EntityLivingBase entityTarget,
			float attackDamage) {

		int knockbackModifier = 0;

		attackDamage += EnchantmentHelper.getModifierForCreature(entityAttacker.getHeldItemMainhand(),
				entityTarget.getCreatureAttribute());
		...
		if (wasAttacked) {
			...
			if (knockbackModifier > 0) {
				entityTarget.knockBack(entityAttacker, (float) knockbackModifier * 0.5f,
			...
			if (entityTarget instanceof EntityPlayer) {
				...
				if (!heldItem.isEmpty() && !activeItem.isEmpty()
						&& heldItem.getItem().canDisableShield(heldItem, activeItem, entityplayer, entityAttacker)
