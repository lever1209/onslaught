...
public class InvasionPlayerDataFactory {
	...
	public InvasionPlayerDataFactory(Function<String, InvasionTemplate> invasionTemplateFunction,
	...
	public InvasionPlayerData.InvasionData create(String templateId, UUID invasionUuid, Random random, long timestamp,
			long maxInvasionDuration, long warningMessageTimestamp) {

		InvasionTemplate invasionTemplate = this.invasionTemplateFunction.apply(templateId);

		if (invasionTemplate == null) {
			ModOnslaught.LOG.error("Unknown template id: " + templateId);
			return null;
		}
	 ...
	 * @return an {@link InvasionPlayerData.InvasionData.MobData} generated from the
	 *         given {@link InvasionTemplateWave.Mob} and {@link Random}
	 */
	private InvasionPlayerData.InvasionData.MobData createMobData(InvasionTemplateWave.Mob mob, boolean forceSpawn,
			Random random) {
	 ...
	 * @param random the {@link Random} prng
	 * @return an {@link InvasionTemplateWave.Group} selected using a
	 *         {@link WeightedPicker} and the given {@link Random} prng
	 */
	 ...
	 * @param random the {@link Random} prng
	 * @return a random value in the given range, accounting for fixed, single value
	 *         arrays
	 */
