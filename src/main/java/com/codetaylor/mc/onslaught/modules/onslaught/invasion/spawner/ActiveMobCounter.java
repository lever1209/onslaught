...
/**
 * Responsible for counting active mobs for a specific player, wave, and mob
 * type.
 */
public class ActiveMobCounter {
	...
	public int countActiveMobs(List<Entity> entityList, UUID invasionUuid, UUID playerUuid, int waveIndex,
			int mobIndex) {
		...
		for (Entity entity : entityList) {
			...
			if (modTag.hasKey(Tag.INVASION_DATA)) {
				...
				if (waveIndex == waveIndexData && mobIndex == mobIndexData && playerUuidString.equals(playerUuidData)
		...
		for (DeferredSpawnData deferredSpawnData : this.deferredSpawnDataList) {
			...
			if (waveIndex == deferredSpawnData.getWaveIndex() && mobIndex == deferredSpawnData.getMobIndex()
