...
public class InvasionKillCountUpdater {
	...
	public void onDeath(InvasionGlobalSavedData invasionGlobalSavedData, NBTTagCompound entityData,
		...
		if (invasionData != null && invasionData.getInvasionUuid().equals(invasionUuid)) {
			...
			if (ModuleOnslaughtConfig.DEBUG.INVASION_DATA_UPDATES) {
				String message = String.format("Kill count updated for wave index %d mob id %s from %d to %d",
						waveIndex, mobData.getMobTemplateId(), killedCount, killedCount + 1);
				ModOnslaught.LOG.debug(message);
			}

			MinecraftForge.EVENT_BUS
					.post(new InvasionEntityKilledEvent(playerFunction.apply(playerUuid), invasionData));
		}
	}
}
